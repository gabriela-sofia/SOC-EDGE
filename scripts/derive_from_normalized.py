#!/usr/bin/env python3
"""
Derive Stage 4C by reading Method B parts directly.
Simplified: no deltas, just essential columns.
"""
...
print("Expected rows: " + str(expected))
print("Quality OK files: " + str(len(quality_ok)))

print("\nProcessing Method B parts...")
...
for i, part in enumerate(parts):
    ...
    # Derive Stage 4C
    df_out = df[['dataset', 'cell_id', 'profile_type', 'temperature_condition', 'cycle_id', 'source_file', 'time_s', 'voltage_V', 'temperature_C']].copy()
    ...
    df_out.to_parquet(Path(OUTPUT_PARQUET) / ("part_" + str(part_idx) + ".parquet"), index=False, compression='snappy')
...
print("Rows after filtering: " + str(len(df)))

# Derive Stage 4C
df_out = df[['dataset', 'cell_id', 'profile_type', 'temperature_condition',
             'cycle_id', 'source_file', 'time_s', 'voltage_V', 'temperature_C']].copy()
...
# Derive deltas per source_file + cycle_id
for (src, cyc), idx_list in df.groupby(['source_file', 'cycle_id'], sort=False).groups.items():
...
print("Saving as single parquet...")
Path(OUTPUT_PARQUET).mkdir(parents=True, exist_ok=True)
df_out.to_parquet(Path(OUTPUT_PARQUET) / "part_0.parquet", index=False, compression='snappy')

# Report
generated = len(df_out)
match_pct = (generated / expected * 100) if expected > 0 else 0
...
with open("outputs/external_datasets/lg18650_stage4c_feature_summary.md", 'w') as f:
    ...
    f.write("Expected rows: " + str(expected) + "\n")
    f.write("Generated rows: " + str(generated) + "\n")
    f.write("Reconciliation: " + str(round(match_pct, 1)) + "%\n\n")
    f.write("Files processed: 1 consolidated\n")
    f.write("Parquet parts: 1\n\n")
...
with open("outputs/external_datasets/lg18650_stage4c_feature_reconciliation.md", 'w') as f:
    ...
    if decision != "LG_STAGE4C_FEATURES_READY":
        f.write("Missing: " + str(expected - generated) + " rows\n")
        f.write("Cause: Check if normalized.parquet contains all expected rows\n")
